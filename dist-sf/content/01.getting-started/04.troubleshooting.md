...
- **Use absolute URLs**: Images should use full URLs (e.g., `https://example.com/image.png`)
- **Check URL accessibility**: Verify the image URL is accessible in a browser
- **Check file extensions**: Ensure image extensions match (case-sensitive: `.jpg` vs `.JPG`)
- **Verify signed URLs**: If using private storage, ensure URLs are signed or authenticated

### Image Format Issues

- **Supported formats**: PNG, JPG, JPEG, GIF, SVG, WebP
- **SVG considerations**: SVG files may need special handling in some browsers
- **Case sensitivity**: File paths are case-sensitive - `image.PNG` ≠ `image.png`

### Experience Cloud CSP Issues

If images load on internal Lightning pages but not in Experience Cloud, you likely have a Content Security Policy (CSP) issue.

**Symptoms:**
- Images show as broken in Experience Cloud
- Console shows "violates Content Security Policy directive: img-src"

**Solution:**
1. Go to **Setup** → **CSP Trusted Sites**
2. Click **New Trusted Site**
3. Configure:
   - **Name**: Descriptive name (e.g., "GitHub_Images")
   - **Trusted Site URL**: Your image hosting domain (e.g., `https://raw.githubusercontent.com`)
   - **Context**: Select "Communities" or "Experience Builder Sites"
   - **Directives**: Enable **img-src** (and **media-src** for videos)
4. Save

> [!TIP]
> For GitHub-hosted images, you need to allow `https://raw.githubusercontent.com` (where raw files are served) and optionally `https://github.com` for blob URLs.
...
> 📚 **Next**: Review [Configuration](/core-concepts/configuration) options or learn about [Contributing](/getting-started/contributing).
