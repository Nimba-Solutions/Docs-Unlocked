...
```mermaid
classDiagram
    class OrderLookupCtrl {
        +getLineItemsByOrderNumber()
        +decryptData()
        +createTesterDetail()
        +createLabOrder()
        +createCustomerUser()
    ...
    class MinorOrderRegistrationCtrl {
        +getLineItemsByOrderNumber()
        +decryptData()
        +createTesterDetail()
        +updateMinorDetails()
    ...
    class OrderRegistrationUIWrapper {
        +orderInfo
        +labOrderId
        +isService
    }
    
    class DynamicAOEComponentHelper {
        +insertTesterAoeRecords()
        +getAoeQuestionsAndOptions()
        +getProductCodes()
    }
    
    class CreateLabPWNServiceOrder {
        +createPWNOrder()
    }
    
    class CreateLabHUBServiceOrder {
        +createHubOrder()
    }
    
    OrderLookupCtrl --> OrderRegistrationUIWrapper : creates
    OrderLookupCtrl --> TestCodes : uses
    MinorOrderRegistrationCtrl --> OrderRegistrationUIWrapper : creates
    MinorOrderRegistrationCtrl --> TestCodes : uses
    OrderController --> OrderService : delegates
    OrderService --> TestCodes : uses
    OrderRegistrationUIWrapper --> ProductData : contains
```
````
