...
class ConditionRegistry {
     ...
     */
    register(type: string, evaluator: ConditionEvaluator, parser?: ConditionParser): void {
...
/**
 * Permission condition (defined here for type exports, implemented in permissionConditions.ts)
 */
export interface PermissionCondition extends Condition {
    type: 'permission';
    permissionType: 'permission' | 'custom-permission' | 'permission-set' | 'profile' | 'object' | 'field';
    value: string;
    access?: string; // For object access: read, edit, create, delete, viewAll, modifyAll
}

/**
 * Evaluate a logical condition (all, any, not)
 */
export async function evaluateLogicalCondition(condition: Condition): Promise<ConditionResult> {
    const logical = condition as LogicalCondition;
    if (logical.conditions.length === 0) {
        return { shouldShow: true, isValid: true };
    }

    const results = await Promise.all(logical.conditions.map((cond) => conditionRegistry.evaluate(cond)));
    ...
    switch (logical.operator) {
        ...
        case 'not':
            // 'not' only uses the first condition
...
// Register only the logical condition evaluator here
// The permission evaluator and parser are registered in permissionConditions.ts
